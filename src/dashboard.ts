...
export class DashboardProvider implements vscode.WebviewViewProvider, vscode.Disposable {
  ...
  resolveWebviewView(view: vscode.WebviewView): void {
    ...
    view.webview.onDidReceiveMessage(message => {
      ...
      if (message?.type === "addSkill") void vscode.commands.executeCommand("agent-tool.addSkill");
...
function dashboardHtml(webview: vscode.Webview): string {
  ...
  return `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src ${webview.cspSource} 'unsafe-inline'; script-src 'nonce-${nonce}';"><meta name="viewport" content="width=device-width,initial-scale=1"><style>
    ...
    .bar { display:flex; gap:7px; align-items:center; margin:0 0 12px; } .bar button:first-child { margin-left:auto; }
  ...
  </style></head><body><header><div><h1>Agent Tool</h1><div class="sub">AI agent tools in this workspace</div></div><button class="icon" title="再読み込み" id="refresh">↻</button></header><div class="overview" id="overview"></div><div class="bar"><strong>Inventory</strong><button id="skill">＋ Skill</button><button id="mcp">＋ MCP</button></div><div class="filters" id="filters"></div><div id="content"></div><script nonce="${nonce}">
  const vscode = acquireVsCodeApi(); let items = []; let scope = 'all'; let kind = 'all';
  const kinds = {skill:'Skill',subagent:'Subagent',mcp:'MCP',plugin:'Plugin'}; const icons = {skill:'◇',subagent:'⌘',mcp:'◉',plugin:'▦'};
  const esc = s => String(s).replace(/[&<>'"]/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));
  function render() { const visible=items.filter(x=>(scope==='all'||x.scope===scope)&&(kind==='all'||x.kind===kind)); const managed=items.filter(x=>x.origin!=='bundled').length, updates=items.filter(x=>x.hasUpdate).length;
    ...
    const groups=[['Your tools',visible.filter(x=>x.origin!=='bundled')],['Bundled',visible.filter(x=>x.origin==='bundled')]]; document.querySelector('#content').innerHTML=visible.length?groups.filter(([,rows])=>rows.length).map(([title,rows])=>'<div class="section">'+title+'</div><div class="list">'+rows.map((x,i)=>'<div class="item"><div class="glyph">'+icons[x.kind]+'</div><div><div class="name">'+esc(x.name)+'</div><div class="meta">'+esc(x.agents.join(' · '))+' · '+kinds[x.kind]+'</div></div><button class="icon action" data-index="'+items.indexOf(x)+'" title="操作">•••</button></div>').join('')+'</div>').join(''):'<div class="empty">この条件に一致するツールはありません。</div>';
    document.querySelectorAll('[data-filter]').forEach(b=>b.onclick=()=>{const v=b.dataset.filter; if(v==='all'||v==='project'||v==='user') scope=v; else kind=v; render();}); document.querySelectorAll('.action').forEach(b=>b.onclick=()=>vscode.postMessage({type:'actions',item:items[Number(b.dataset.index)]})); }
  document.querySelector('#refresh').onclick=()=>vscode.postMessage({type:'refresh'}); document.querySelector('#skill').onclick=()=>vscode.postMessage({type:'addSkill'}); document.querySelector('#mcp').onclick=()=>vscode.postMessage({type:'addMcp'}); window.addEventListener('message',e=>{if(e.data.type==='inventory'){items=e.data.items;render();}}); render();
  </script></body></html>`;
}

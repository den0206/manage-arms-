  ...
  implements vscode.WebviewViewProvider, vscode.Disposable
{
  ...
  private post(items: DashboardItem[]): void {
    ...
    const projectName = folders?.[0]?.uri.fsPath.split('/').pop() ?? 'Current Project';
    this.view?.webview.postMessage({type: 'inventory', items: annotated, projectName});
  }
}
...
function dashboardHtml(webview: vscode.Webview): string {
  ...
  const vscode = acquireVsCodeApi(); let items = []; let agent = ''; let scope = 'project'; let projectName = 'Current Project'; let sectionExpanded = {}; let selected = '';
  ...
  function render() { const agents=Object.keys(agentNames).filter(a=>items.some(x=>x.agents.includes(a))); if(!agents.includes(agent)) agent=agents[0]??''; const visible=items.filter(x=>x.agents.includes(agent)&&x.scope===scope); const managed=items.filter(x=>x.origin!=='bundled').length, updates=items.filter(x=>x.hasUpdate).length;
    ...
    document.querySelector('#agents').innerHTML=agents.length?agents.map(a=>'<button class="agent '+(a===agent?'active':'')+'" data-agent="'+a+'" role="tab" aria-selected="'+(a===agent)+'"><span class="agent-dot"></span><span>'+agentNames[a]+'</span><span class="agent-count">'+items.filter(x=>x.agents.includes(a)).length+'</span></button>').join(''):'<p class="empty">どの AI Agent にもツールが導入されていません。上の URL 欄からツールを追加してください。</p>'; const _cnt=items.filter(x=>x.agents.includes(agent)).length;const _ab=document.querySelector('#agent-badge');if(_ab)_ab.innerHTML=agent?'<span class="badge-pip" data-agent="'+agent+'"></span><span class="badge-name">'+agentNames[agent]+'</span><span class="badge-count">'+_cnt+' tool'+(_cnt===1?'':'s')+'</span>':'';
    const filters=[['user','User Global'],['project',projectName]]; document.querySelector('#filters').innerHTML=filters.map(([v,n])=>'<button class="filter '+(v===scope?'active':'')+'" data-filter="'+v+'">'+n+'</button>').join('');
    const rowsHtml=rows=>'<div class="list">'+rows.map(x=>'<div class="item"><div class="glyph" data-kind="'+x.kind+'">'+icons[x.kind]+'</div><div><div class="name">'+esc(x.name)+'</div><div class="meta">'+esc(x.agents.join(' · '))+' · '+kinds[x.kind]+(x.kind==='mcp'?' · '+(x.running?'Running':'Stopped'):'')+'</div></div><button class="icon action" data-index="'+items.indexOf(x)+'" title="操作">•••</button></div>').join('')+'</div>'; const yours=visible.filter(x=>x.origin!=='bundled'); const yourGroups=Object.entries(kinds).map(([value,title])=>[title,yours.filter(x=>x.kind===value),value]).filter(([,rows])=>rows.length); const bundled=visible.filter(x=>x.origin==='bundled'); document.querySelector('#content').innerHTML=visible.length?yourGroups.map(([title,rows,kv])=>{const exp=sectionExpanded[kv]; const shown=exp?rows:rows.slice(0,5); const rest=rows.length-shown.length; const tog=rows.length>5?'<button class="section-toggle" data-kind="'+kv+'">'+(exp?'⏄ Show less':'› '+rest+' more')+'</button>':''; return '<div class="section">'+title+'</div>'+rowsHtml(shown)+tog;}).join('')+(bundled.length?(()=>{const exp=sectionExpanded['bundled']; const shown=exp?bundled:[]; const rest=bundled.length; const tog='<button class="section-toggle" data-kind="bundled">'+(exp?'⏄ Show less':'› '+rest+' more')+'</button>'; return '<div class="section">Bundled</div>'+rowsHtml(shown)+tog;})():''):'<div class="empty">この条件に一致するツールはありません。</div>';

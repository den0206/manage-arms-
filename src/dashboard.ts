...
function dashboardHtml(webview: vscode.Webview): string {
  ...
  const vscode = acquireVsCodeApi(); const T = ${text}; let items = []; let issues = []; let loadError = ''; let agent = ''; let scope = 'project'; let projectName = 'Current Project'; let sectionExpanded = {}; let onlyUpdates = false; let selected = ''; let projects = []; let otherPath = ''; let otherItems = []; let otherLoading = false; let otherError = ''; let lastPreview = null; let environment = []; let readOnly = ''; let otherIssues = [];
  ...
  function render() { const agents=Object.keys(agentNames).filter(a=>items.some(x=>x.agents.includes(a))); if(!agents.includes(agent)) agent=agents[0]??''; const managed=items.filter(x=>x.origin!=='bundled').length, updates=items.filter(x=>x.hasUpdate).length; if(updates===0) onlyUpdates=false;
    ...
    document.querySelector('#overview').innerHTML='<div class="metric"><strong>'+managed+'</strong><span>'+esc(T.yourTools)+'</span></div><div class="metric'+(updates?' clickable':'')+(onlyUpdates?' active':'')+'"'+(updates?' id="updates-metric" role="button" tabindex="0" aria-pressed="'+onlyUpdates+'" title="'+esc(T.showUpdates)+'"':'')+'><strong>'+updates+'</strong><span>'+esc(T.updates)+'</span></div>'+banner();

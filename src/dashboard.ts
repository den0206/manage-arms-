...
function dashboardHtml(webview: vscode.Webview): string {
  ...
  </style></head><body><header><div><h1>Agent Tool</h1><div class="sub">AI agent tools in this workspace</div></div><button class="icon" title="再読み込み" id="refresh">↻</button></header><div class="overview" id="overview"></div><div class="section">AI Agents</div><nav class="agent-nav" id="agents"></nav><div class="bar"><strong id="inventory-title">All tools</strong><button id="skill">＋ Skill</button><button id="mcp">＋ MCP</button></div><div class="filters" id="filters"></div><div id="content"></div><script nonce="${nonce}">

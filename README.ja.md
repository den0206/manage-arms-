...
| **使用実績** | セッションログから最終使用日を集計。表示中は3秒ごとにMCPプロセスを確認。実際のTool呼び出し中とは区別 |
| **ブラウザ検知** | 既定 OFF。ウィンドウを開いたままブラウザで Skill・Plugin・Subagent のページを開くと、画面上部の帯で追加を提案する（URL を貼らずに済み、システム通知は使わない）。出す前に `raw.githubusercontent.com` で実在を確かめ、URL は保存しない |
...
- **ブラウザ検知は既定 OFF・範囲は最小。** ON にした時点でブラウザ制御の許可を求め、
  拒否されたら自分で止まる。動くのはウィンドウが開いていてブラウザが前面の間だけで、
  通知の許可は求めない（見つけたものはウィンドウ内に出す）。

...
| **有効 / 無効 / 削除** | 無効化は実体を退避するだけ。削除は実体をゴミ箱へ移すので、どちらも必ず戻せる |
| **取り込み** | 自分で入れたスキルを、ファイルを動かさずにこのアプリの管理下へ移せる |
| **更新** | `SKILL.md` の差分を見てから適用する。上流の方針転換に備えてピン留めもできる |
| **スコープ** | 「ユーザー全体」「プロジェクトごと」「エージェント同梱」をタブで切り替える。両方に入っていれば重複として両方に警告 |

...
| **ブラウザ検知** | 既定 OFF。ウィンドウを開いたままブラウザで Skill・Plugin・Subagent のページを開くと、URL を貼らずに追加を提案する。通知の前に `raw.githubusercontent.com` で実在を確かめ、URL は保存しない |
...
- **ブラウザ検知は既定 OFF・範囲は最小。** ON にした時点でブラウザ制御の許可を求め、
  拒否されたら自分で止まる。動くのはウィンドウが開いていてブラウザが前面の間だけ。

...
| **更新** | 全ファイルの変更一覧と上限付きの本文差分を確認して適用。同じ内容なら確認済みの版だけ記録。固定すると更新を止められる |
| **スコープ** | 「ユーザー全体」「プロジェクトごと」「エージェント同梱」をタブで切り替える。重複は両方に警告し、プロジェクト側だけ一括削除できる |
| **使用実績** | Claude Code・Codex・Cursor のセッションログから最終使用日を集計。表示中は3秒ごとにMCPプロセスを確認。実際のTool呼び出し中とは区別  途中で止まった分析は次回続きから再開 |
...
```bash
swift test                  # 単体テスト（@Test 定義 379 件）
swift build                 # コンパイル確認

...
✳️ Claude Code    3     このエージェントが持っているもの
➤ Cursor          2
</> Codex          1
✦ Gemini CLI     未検出
...
- **アイコンはエージェントごとに変える。** 同じ記号が 4 つ並ぶと、
  選択中がどれか一目で分からない（`Agent.symbol` / `Agent.tint`）
...
| 角丸 | 7（小物: チップ・入力欄）/ 12（カード） | |
| `Theme.brand` | アクセント → 紫のグラデーション | **ヒーローと選択中のチップだけ。** 差し色は薄く効かせる |
| `card()` | 境界線 + ごく淡い影 | ホーム・エージェントカード・候補行 |
...
どこで見つける？
  anthropics/skills — Anthropic 公式のスキル集
  GitHub の claude-skills トピック

エージェントの検出状況
  Claude Code  2.1.236  /opt/homebrew/bin/claude
  Cursor       ~/.cursor（CLI なし・設定を直接読む）
  Gemini CLI   未検出
```

**Cursor は CLI が無い**（`cursor-agent` は存在しない）ので、空欄にせず理由を書く。
CLI が無いこと自体は異常ではないので「未検出」とは表示しない（3.7 の 4 状態）。

`PATH` 解決に失敗した時の逃げ道（**手動でパスを指定**）はまだ入れていない。
3.7 の通り GUI アプリの `PATH` は当てにならないため、詰む環境が出たら足す。

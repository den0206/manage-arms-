...
| エージェントのディスク使用量の**掃除**機能 | このアプリの仕事ではない。**量は見せる**（8 章）が、消すかどうかは利用者が各行の削除で決める |
...
スカラー値（文字列・bool、TOML は整数・小数も）を Config Inspector タブから読み書きする。
...
- **TOML**: 更新は値の範囲だけを差し替え、追加・削除は対象行のみ。コメント・空白・改行を保持する。
  ドット付きキーとセクションを合わせた完全パスで識別し、引用符内のドットは名前の一部として扱う。
  保存前後に対応構文を解析し、重複キー・テーブルの再定義・型の変化・対象外の値や行の変更を拒否する。
- **競合検出**: 書き込み直前にファイルを再読し、ロード時と一致しなければ中断
- **アトミック書き込み**: `Data.write(to:options:.atomic)`

Config Inspector は主要なスカラー設定を未設定のものも含めて表示し、検索・説明・
公式リファレンスへのリンクを提供する。key 名・値・項目の英語説明は翻訳しない。
`ConfigField` は主要項目の入力ヒントだけを持ち、未知の対応スカラーは「その他の設定」で
編集できる。選択肢にない既存値も保持する。文字列は保存ボタンまたは Enter、選択肢と
bool は変更時に保存する。未設定は実効デフォルトを意味せず、このファイルに値がない状態。
TOML の数値は 10 進の Int64 と有限の小数（指数・桁区切りを含む）。小数は文字列表現を保持し、
精度を丸めず数値リテラルとして保存する。型は編集時に変更しない。JSON の配列・数値・深い
オブジェクト、TOML の配列・インラインテーブル・配列テーブルはフォーム編集の対象外。
単一行コンテナと配列テーブルは検証・保持する。複数行値・日時・非10進数・未対応の文字列
エスケープなどがあればファイル全体の読み書きを拒否し、外部エディタへ案内する。
完全な TOML パーサや Agent のスキーマ検証ではなく、検証できる構文だけを編集する設計。
key を追加しても固定リストの更新は不要。操作文言だけをローカライズする。
`hooks` とその配下は表示対象から除外し、設定ファイル内の内容は保持する。
参考: [Claude settings](https://code.claude.com/docs/en/settings) /
[Codex config reference](https://developers.openai.com/codex/config-reference)（2026-09-08 確認）。
...
**ホーム自身とルートはプロジェクトとして扱わない**（`ProjectScan.isProject`）。
`claude` をホームで一度でも起動すると `projects` に `/Users/<me>` が載る。これを信じると
下の `skillRoots` がホーム配下を深さ 3 まで歩き、**他の全プロジェクトの `.claude/skills` と
`~/.claude/skills` 自身が「ホーム」という 1 つの偽プロジェクトに吸い込まれる**。
実測: 27 ルート・350 件が 1 タブに入り（うち 24 は git worktree の重複）、
そのタブを開くと一覧が描き切れずアプリが応答を失った。
3.4 が踏まないと決めた広さそのものなので、`projects` の値であっても除外する。

**除外したことは設定画面に出す**（`ProjectScan.ignoredProjects`）。黙って捨てると
「登録したのに一覧に出ない」が原因不明のまま残る。**ただし削除の導線は置かない** —
登録を消せる唯一の手段は `claude project purge <path>` で、ホームは全プロジェクトの
パスの親なので配下の transcript まで巻き添えに消える（実測: エントリ 1 件を消すために
19 項目・7 プロジェクト分の transcript と memory/・10 セッションの file-history・
487 プロンプトが削除対象になった）。5.2 の「何が消えるか分からないボタンを作らない」に
反するので、事実だけ出して判断は利用者に返す。アプリ自身は `~/.claude.json` を
書かない（不変条件 1・`WriteGuard.deniedNames`）。
...
（実機で確認）ので、図案ごと色を変える。更新の知らせ（7.6）は図案の**外**に点を並べる —
同じ図案を共有すると、開くまでどちらの知らせか分からない。
...
**復元にも失敗したときは退避先を消さない。** 旧実体はそこにしか無く、ゴミ箱も
経由しないので、畳むと戻す手段が一つも残らない。エラー文言に退避先のパスを出す。
...
**更新があるものが 1 つでもあれば、メニューバーアイコンの右横に緑の点を出す。**
判断材料は `registry.json` だけ（`UpdateChecker.hasAvailable`）なので、走査もネットワークも
要らない — つまり**常駐しない**（3.5）に触れない。逆に言えば見えるのは
**最後に「更新を確認」した時点の遅れ**であって、常駐中に勝手に確認しに行きはしない（7.3）。
**固定中は数えない** — 遅れていても更新しないと利用者が決めたものなので（7.4）。

件数は出さない。数字を出すと「いくつ残っているか」を追う画面になり、
点で足りる（検知の緑と同じ判断、6 章）。**検知の緑とは場所を分ける** —
検知は図案ごと緑、更新は図案の外の点。同じ見た目にすると、
開いてみるまでどちらか分からない。
...
スキル・サブエージェントを追加する
  ...
  MCP 1 │ スキル 10 │ サブエージェント 3 │ プラグイン 4 │ ストレージ 123.4 MB
...
**規模（件数）の右隣に量（容量）を置く。** 帯に載るのは**自分で入れたものだけ**で、
同梱は数えない — 消せないものの容量を見せても打つ手が無い。内訳は各エージェント画面の
行ごとに出す（合計だけ見せて内訳を隠すと、どれが重いか分からず削除の判断に使えない）。

**容量は「実体」を数える。** このアプリは各エージェントから見えるように symlink を張るので
（3.2）、素朴に足すと同じ実体を何度も数える。実体パスで重複を落としてから測り、
symlink そのものは 0 として扱う（`du` と同じ見え方）。逆に、同じプラグインが Claude と
Codex の両方に入っていれば**実体が 2 つあるので 2 回数える** — これは重複の見せ場（5.2）と
同じ事実を量で言い直したものになる。

**0 のときは数字を出さない。** MCP はコマンドで起動するだけでローカルに実体を持たず、
測れなかったものも 0 になる。**この 2 つを 0 という数字に潰すと嘘になる**ので、
どちらも「—」に落とす。数字を出せるのは実際に測れたものだけ。
...
| 18 | 更新バッジ（7.6）を出している間、図案が明・暗とメニュー選択中で見えるか | 点は色なので画像を非テンプレートにするしかなく、17 で消したはずの外観依存が図案側に戻る。図案は `labelColor` を `cacheMode = .never` で描き直しているが、**テンプレートと違いメニュー選択中に反転しない**。沈むなら検知と同じく図案ごと色を変えるか、点だけの別画像にするかの判断 |
...
- プロジェクト設定の走査対象は `~/.claude.json` の `projects` キーだけから採る
  （`ProjectScan.projectPaths`）。ホーム全体を再帰走査しない。**`projects` にホーム自身や
  `/` が載っていても、それはプロジェクトとして扱わない**（5.1）。**手で足す導線は置かない** —

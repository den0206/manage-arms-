...
- **状態**: **v1〜v5 実装済み**（Skills / Subagents / Plugins / 使用実績 / MCP。v4 の権限は撤去）、
  および **配布基盤**（`.app` 組み立て / 署名・公証 / DMG / CI）。テスト定義 374 件
...
行差分は入力合計 1 MiB・出力 2,000 行・比較対象 1,000 行/ファイルまでに制限する。
上限を超えてもファイル単位の変更一覧は残し、本文の省略を表示する。
取得物と現行の manifest が同じなら、実体を差し替えず確認済み SHA だけを保存する。
...
退避の失敗は差し替え失敗の rollback に入れず、元の実体を削除しない。
プレビュー作成の例外・キャンセルでも、一時領域の所有権を渡す前なら破棄する。
...
- **`registry.json` はアトミックに書く** — 唯一の永続ファイルなので、
  ...
  merge する。導入・有効化・無効化・削除も対象リソースだけを更新し、
  読み込み時点の Registry 全体を保存し直さない

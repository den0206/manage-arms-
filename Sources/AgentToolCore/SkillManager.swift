...
enum ManagedLifecycle {
    ...
    static func restore(_ entry: Registry.Entry, from trash: URL, originalPath: URL,
                        env: Environment) throws -> URL {
        guard let kind = Kind(rawValue: entry.kind) else { throw SkillManager.Failure.notFound(entry.name) }
        try WriteGuard.assertValidName(entry.name)
        try Registry.assertReadable(env: env)
        let layout = layout(entry.name, kind: kind, env: env)
        let destination = entry.disabled ? layout.parked : layout.store
        guard originalPath.standardizedFileURL == destination.standardizedFileURL,
              WriteGuard.isInside(trash, env.home.appending(path: ".Trash")),
              FileManager.default.fileExists(atPath: trash.path),
              !FileManager.default.fileExists(atPath: destination.path) else {
            throw SkillManager.Failure.notFound(entry.name)
        }
        try FileManager.default.createDirectory(at: destination.deletingLastPathComponent(), withIntermediateDirectories: true)
        try FileManager.default.moveItem(at: trash, to: destination)
        ...
        if !entry.disabled {
            ...
            try registry.save(env: env)
            try enable(entry.name, kind: kind, env: env, registry: &registry)
        }
        return destination
    }
}

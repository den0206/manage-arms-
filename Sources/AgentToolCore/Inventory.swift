...
public struct ResourceRow: Identifiable, Sendable {
    ...
    /// 出どころ（）。**「自分で入れたもの」と「最初から入っていたもの」を混ぜない。**
    ...
    ///
    /// **1 件ずつの削除は別経路**（`SkillManager.removeExisting`、）。
    ...
    /// 。`inherited` はプロジェクトスコープ表示用（v1 では未使用）。
...
public struct Inventory: Sendable {
    ...
    public static func restore(_ entry: Registry.Entry, from trash: URL, originalPath: URL,
                               env: Environment) throws -> URL {
        try ManagedLifecycle.restore(entry, from: trash, originalPath: originalPath, env: env)
    }
...
public struct ProjectScan: Sendable {
    ...
    /// 旧版が registry に残したプロジェクトだけを使う（ / 15）。

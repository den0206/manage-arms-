...
/// DESIGN.md 8 章 — 自分で入れたものをこのアプリの管理下に取り込む。
/// **ファイルは動かさない。** registry に足して symlink を張るだけ。
@Suite("管理下への取り込み")
struct AdoptTests {

    func row(_ name: String, kind: Kind = .skill, roots: [String],
             origin: ResourceRow.Origin = .user) -> ResourceRow {
        ResourceRow(name: name, kind: kind, summary: nil, detail: "",
                    state: [.claude: .explicit], origin: origin, isDisabled: false, roots: roots)
    }

    @Test("置き場にあるものだけが取り込める")
    func classifies() {
        #expect(row("a", roots: [".agents/skills"]).adoption == .possible)
        #expect(row("b", kind: .subagent, roots: ["agents"]).adoption == .possible)
        // 実体が別の場所にあるものは移動が要るのでやらない。
        #expect(row("c", roots: [".claude/skills"]).adoption == .needsMove(".claude/skills"))
        #expect(row("d", kind: .mcp, roots: []).adoption == .unsupported)
        #expect(row("e", kind: .plugin, roots: []).adoption == .unsupported)
        #expect(row("f", roots: [".cursor/skills-cursor"], origin: .bundled).adoption == .unsupported)
        #expect(row("g", roots: [".agents/skills"], origin: .managed).adoption == .unsupported)
    }

    @Test("取り込むと registry に載り、symlink が張られ、消せるようになる")
    func adopts() throws {
        let f = try SkillManagerTests.Fixture()
        // 他ツールが置いた体で、実体だけを置き場に用意する（registry には無い）。
        let dir = f.store("outsider")
        try f.fm.createDirectory(at: dir, withIntermediateDirectories: true)
        try "---\nname: outsider\ndescription: d\n---\n"
            .write(to: dir.appending(path: "SKILL.md"), atomically: true, encoding: .utf8)
        ...
        #expect(throws: WriteGuard.Denial.notInRegistry("outsider")) {
            try SkillManager.remove("outsider", env: f.env, registry: &registry)
        }

        try SkillManager.adopt("outsider", env: f.env, registry: &registry)
        #expect(registry.entry(named: "outsider") != nil)
        #expect(f.isLink(f.link("outsider")))
        #expect(f.exists(dir))                                  // 実体は動いていない
        #expect(Registry.load(env: f.env).entry(named: "outsider") != nil)

        // 取り込んだ後は削除できる。
        let trashed = try SkillManager.remove("outsider", env: f.env, registry: &registry)
        try trashed.map { try f.fm.removeItem(at: $0) }
    }

    @Test("他ツールの symlink が居座っていれば取り込まない（横取りしない）")
    func refusesForeignLink() throws {
        let f = try SkillManagerTests.Fixture()
        let dir = f.store("codiff")
        try f.fm.createDirectory(at: dir, withIntermediateDirectories: true)
        // 他アプリのバンドルを指す symlink が ~/.claude/skills にある状態。
        try f.fm.createDirectory(at: f.env.claudeSkills, withIntermediateDirectories: true)
        try f.fm.createSymbolicLink(at: f.link("codiff"),
                                    withDestinationURL: f.env.home.appending(path: "Applications/X.app"))

        var registry = Registry()
        #expect(throws: (any Error).self) {
            try SkillManager.adopt("codiff", env: f.env, registry: &registry)
        }
        #expect(registry.entry(named: "codiff") == nil)   // 半端に登録が残らない
    }
}

...
struct ManagementTests {
    ...
    func corruptRegistry() throws {
        let env = try fixture(["Library/Application Support/AgentTool/registry.json": "broken"])
        defer { try? FileManager.default.removeItem(at: env.home) }
    ...
    @Test("Mac App専用設定は読み飛ばして再保存しない")
    func dropsLegacyAppSettings() throws {
        let old = Data(#"{"resources":[],"browserDetection":true,"menuBar":true,"appearance":"dark"}"#.utf8)
        let registry = try Registry.decoder.decode(Registry.self, from: old)
        let encoded = String(decoding: try Registry.encoder.encode(registry), as: UTF8.self)
        #expect(!encoded.contains("browserDetection"))
        #expect(!encoded.contains("menuBar"))

...
struct SkillRemoveTests {
    ...
    @Test("ゴミ箱から復元すると実体・symlink・registryが戻る")
    func restores() throws {
        var f = try SkillManagerTests.Fixture()
        try f.install("demo")
        let entry = try #require(f.registry.entry(named: "demo", kind: .skill))
        let trash = f.env.home.appending(path: ".Trash/demo")
        try f.fm.createDirectory(at: trash.deletingLastPathComponent(), withIntermediateDirectories: true)
        try f.fm.removeItem(at: f.link("demo"))
        try f.fm.moveItem(at: f.store("demo"), to: trash)
        f.registry.resources.removeAll { $0.name == "demo" && $0.kind == Kind.skill.rawValue }
        try f.registry.save(env: f.env)

        let restored = try Inventory.restore(entry, from: trash, originalPath: f.store("demo"), env: f.env)
        #expect(restored == f.store("demo"))
        #expect(f.exists(f.store("demo")))
        #expect(f.isLink(f.link("demo")))
        #expect(Registry.load(env: f.env).entry(named: "demo", kind: .skill) == entry)
    }
